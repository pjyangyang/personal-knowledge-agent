...
def stream_answer(question: str, citations: list[CitationRead]):
    if not citations or not settings.llm_api_key or not settings.llm_model:
        yield generate_answer(question, citations)
        return
    client = OpenAI(api_key=settings.llm_api_key, base_url=settings.llm_base_url)
...
def build_messages(question: str, citations: list[CitationRead]) -> list[dict[str, str]]:
    evidence = "\n\n".join(
